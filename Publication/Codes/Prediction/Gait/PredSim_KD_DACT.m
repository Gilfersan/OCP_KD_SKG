...
% Date: 1/7/2019
%
% Edited by Gilmar Fernandes dos Santos on 28.11.2023
clear all;
clc
close all;
...
idx_ww = [805]; % Index row in matrix settings (1:198) idx_ww = [x;y];
...
if ispc    
    switch setup.derivatives
        case {'AD'}   
            if cm == 1
                F = external('F','PredSim_KD1.dll');   
                if analyseResults
                    F1 = external('F','PredSim_KD1_pp.dll');
                end
            elseif cm == 2
                F = external('F','PredSim_KD1_SSCM.dll');   
                if analyseResults
                    F1 = external('F','PredSim_KD1_SSCM_pp.dll');
                end
            else
                name_Fext = ['F = external(''F'',''PredSim_KD1_SSCM',num2str(cm),'.dll'');'];
                eval(name_Fext);
                if analyseResults
                    name_Fext1 = ['F1 = external(''F'',''PredSim_KD1_SSCM',num2str(cm),'_pp.dll'');'];
                    eval(name_Fext1);
                end
            end
    end
...
rheoOr.r   = 30:31;
tibiaOr.l   = 32:33;
tibiaOr.all = [rheoOr.r,tibiaOr.l];
...
foot2Orall.r     = 28:30;
calcOrall.l     = 31:33;
calcOrall.all   = [foot2Orall.r,calcOrall.l];
...
musi = MuscleIndices_Gil4(muscleNames);
...
MTparameters_1 = [MTparameters];
clear MTparameters
load([pathmusclemodel,'/MTparameters_mod_KD_DACT_2_c4.mat']);
MTparameters_2 = [MTparameters];
clear MTparameters
MTparameters_m = [MTparameters_1(:,1:46),MTparameters_2(:,47:76)];
...
[~,mai] = MomentArmIndices_Gil5(muscleNames,...
...
joints = {'pelvis_tilt','pelvis_list','pelvis_rotation','pelvis_tx',...
    ...
    'knee_angle_l','rheo_angle_r','ankle_angle_l',...
    'subtalar_angle_l','foot_angle_r','mtp_angle_l',...
...
elseif IGm == 7
nametrial_walk.IK   = 'IK_IG_KD20_NRh_r';
elseif IGm == 8
nametrial_walk.IK   = 'IK_IG_KD20_NRh_l';
elseif IGm == 9
nametrial_walk.IK   = 'IK_IG_KD17_NRh_r';
elseif IGm == 10
nametrial_walk.IK   = 'IK_IG_KD21_NRh_l';
else
motion_walk         = 'walking';
...
[bounds,scaling] = getBounds_all_Gil19(Qs_walk,NMuscle,nq,jointi,v_tgt);
...
elseif IGsel == 2 % Data-informed initial guess
    if IGm == 1 || IGm == 5 || IGm == 6 || IGm == 7 || IGm == 8 || IGm == 9 || IGm == 10 % Data from average walking motion
        time_IC = [Qs_walk.time(1),Qs_walk.time(end)];
        guess = ...
            getGuess_DI_Gil5(Qs_walk,nq,N,time_IC,NMuscle,jointi,scaling,v_tgt);   
...
if solveProblem
    ...
    % Rheo activations
    rh_a0            = MX.sym('rh_a0',1);
    w               = [w {rh_a0}];
    lbw             = [lbw; bounds.rh_a.lower'];
    ubw             = [ubw; bounds.rh_a.upper'];
    w0              = [w0;  guess.rh_a(1,:)']; 
    ...
    for k=0:N-1
        ...
        % Rheo excitations
        e_rhk                = MX.sym(['e_rh_' num2str(k)], 1);
        w                   = [w {e_rhk}];
        lbw                 = [lbw; bounds.e_rh.lower'];
        ubw                 = [ubw; bounds.e_rh.upper'];
        w0                  = [w0; guess.e_rh(k+1,:)'];
        ...
        rh_akj = {};
        for j=1:d
            rh_akj{j}= MX.sym(['	rh_a_' num2str(k) '_' num2str(j)], 1);
            w       = {w{:}, rh_akj{j}};
            lbw     = [lbw; bounds.rh_a.lower'];
            ubw     = [ubw; bounds.rh_a.upper'];
            w0      = [w0;  guess.rh_a(k+1,:)'];
        end
        ...
        % Left leg
        MA.hip_flex.l   =  MA_rtl(mai(7).mus.l',7);
        MA.hip_add.l    =  MA_rtl(mai(8).mus.l',8);
        MA.hip_rot.l    =  MA_rtl(mai(9).mus.l',9);
        MA.knee.l       =  MA_rtl(mai(10).mus.l',10);
        MA.ankle.l      =  MA_rtl(mai(11).mus.l',11);  
        MA.subt.l       =  MA_rtl(mai(12).mus.l',12); 
        MA.trunk_ext    =  MA_rtl([mai(4).mus.l,mai(4).mus.r]',4);
        MA.trunk_ben    =  MA_rtl([mai(5).mus.l,mai(5).mus.r]',5);
        MA.trunk_rot    =  MA_rtl([mai(6).mus.l,mai(6).mus.r]',6);
        ...
        Tau_passk.knee.l        = f_PassiveMoments(k_pass.knee.l,...
            ...
            Xk_nsc(jointi.knee.l*2,1));
        Tau_passk.rheo.r        = f_PassiveMoments(k_pass.rheo.r,...
            theta.pass.rheo.r,Xk_nsc(jointi.rheo.r*2-1,1),...
            Xk_nsc(jointi.rheo.r*2,1));
        ...
        Tau_passk.subt.l       = f_PassiveMoments(k_pass.subt.l,...
            ...
            Xk_nsc(jointi.subt.l*2,1));
        Tau_passk.prosfoot.r       = f_PassiveMoments(k_pass.prosfoot.r,...
            theta.pass.prosfoot.r,Xk_nsc(jointi.prosfoot.r*2-1,1),...
            Xk_nsc(jointi.prosfoot.r*2,1));
        ...
        Tau_passk_all = [Tau_passk.hip.flex.l,Tau_passk.hip.flex.r,...
            ...
            Tau_passk.knee.l,Tau_passk.rheo.r,Tau_passk.ankle.l,...
            Tau_passk.subt.l,Tau_passk.prosfoot.r,Tau_passk.mtp.l,...
        ...
        % Torque Rheo
        T_Damper_rheo_r = f_DamperRheoKnee(rheo_act,...
            d_damp_rheo,...
            Xk_nsc(jointi.rheo.r*2,1));
        ...
        for j=1:d
            ...
            for r=1:d
                ...
                rh_ap         = rh_ap + C(r+1,j+1)*rh_akj{r};
            end 
            ...
            % Rheo activation dynamics (explicit formulation)  
            drhdt    = f_RheoActivationDynamics(e_rhk,rh_akj{j});
            g       = {g{:}, (h*drhdt - rh_ap)./scaling.rh_a};
            lbg     = [lbg; zeros(1,1)];
            ubg     = [ubg; zeros(1,1)]; 
            ...
            rh_ak_end         = rh_ak_end + D(j+1)*rh_akj{j};
            % Add contribution to the quadrature function
            if W.E == 0
            J = J + 1/(dist_trav_tot)*(...
                W.A*B(j+1)      *(f_J76(akj{j}))*h + ...
                W.Ak*B(j+1)     *(f_J21(Ak(residuals_noarmsi,1)))*h + ... 
                W.passMom*B(j+1)*(f_J15(Tau_passk_all))*h + ...
                W.u*B(j+1)      *(f_J76(vAk))*h + ...
                W.u*B(j+1)      *(f_J76(dFTtildek))*h+ ...
                W.RhE*B(j+1)   *(f_J1(e_rhk))*h);
            
            elseif W.A == 0
            J = J + 1/(dist_trav_tot)*(...
                W.E*B(j+1)      *(f_J76exp(e_tot,exp_E))/body_mass*h + ...                
                W.Ak*B(j+1)     *(f_J21(Ak(residuals_noarmsi,1)))*h + ... 
                W.passMom*B(j+1)*(f_J15(Tau_passk_all))*h + ...
                W.u*B(j+1)      *(f_J76(vAk))*h + ...
                W.u*B(j+1)      *(f_J76(dFTtildek))*h+ ...
        ...
        % Hip flexion, left
        Ft_hip_flex_l   = FTk(mai(7).mus.l',1);
        ...
        ubg             = [ubg; 0];   
        % Hip adduction, left
        Ft_hip_add_l    = FTk(mai(8).mus.l',1);
        ...
        ubg             = [ubg; 0];  
        % Hip rotation, left
        Ft_hip_rot_l    = FTk(mai(9).mus.l',1);
        ...
        % Lumbar extension
        Ft_trunk_ext    = FTk([mai(4).mus.l,mai(4).mus.r]',1);
        ...
        ubg             = [ubg; 0];
        % Lumbar bending
        Ft_trunk_ben    = FTk([mai(5).mus.l,mai(5).mus.r]',1);
        ...
        ubg             = [ubg; 0];
        % Lumbar rotation
        Ft_trunk_rot    = FTk([mai(6).mus.l,mai(6).mus.r]',1);
        ...
        % Torque rheo
        g               = {g{:},Tk(jointi.rheo.r,1)./150 - ...
            (rh_ak + Tau_passk.rheo.r + T_Damper_rheo_r)./150};
        lbg             = [lbg; 0];
        ubg             = [ubg; 0];
        
        % Torque prosankle
        g               = {g{:},Tk(jointi.prosfoot.r,1)./150 - ...
            (Tau_passk.prosfoot.r)./150};
        lbg             = [lbg; 0];
        ubg             = [ubg; 0];
        ...
        g               = {g{:},f_Jnn2(Tk(foot2Or.r,1) - Tk(calcOr.l,1))};
        lbg             = [lbg; 0.0081];
        ubg             = [ubg; 4];   
        % Origins tibia (transv plane) at minimum 11 cm from each other.   
        g               = {g{:},f_Jnn2(Tk(rheoOr.r,1) - Tk(tibiaOr.l,1))};
        lbg             = [lbg; 0.0121];
        ubg             = [ubg; 4]; 
        ...
        % Muscles
        orderMusInv = [1:NMuscle]; %orderMusInv = [NMuscle/2+1:NMuscle,1:NMuscle/2]; 
        orderQsInv = [jointi.pelvis.tilt:2*jointi.pelvis.tz,...
            ...
            2*jointi.rheo.r-1:2*jointi.rheo.r,...
            2*jointi.knee.l-1:2*jointi.knee.l,...
            2*jointi.ankle.l-1:2*jointi.ankle.l,...
            2*jointi.prosfoot.r-1:2*jointi.prosfoot.r,...
        ...
        if k ~= N-1
            ...
            % Rheo activations
            rh_ak            = MX.sym(['rh_a_' num2str(k+1)], 1);
            w               = {w{:}, rh_ak};
            lbw             = [lbw; bounds.rh_a.lower'];
            ubw             = [ubw; bounds.rh_a.upper'];
            w0              = [w0;  guess.rh_a(k+2,:)'];
        else % Periodicty            
            % Muscle activations
            ak              = MX.sym(['a_' num2str(k+1)], NMuscle);
            w               = {w{:}, ak};
        ...
        g   = {g{:}, Xk_end-Xk{k+2,1}, FTtildek_end-FTtildek, ...
            ak_end-ak,rh_ak_end-rh_ak};
        lbg = [lbg; zeros(2*nq.all + NMuscle + NMuscle + 1,1)];
        ubg = [ubg; zeros(2*nq.all + NMuscle + NMuscle + 1,1)];         
    end
...
if analyseResults
    ...
    % All optimized design variables are saved in a single column vector      
    % Number of design variables
    NControls = NMuscle+NMuscle+nq.all+1;
    NStates = NMuscle+NMuscle+2*nq.all+1;    
    NParameters = 1;    
    % In the loop
    Nwl = NControls+d*(NStates)+NStates;
    % In total
    ...
    % Rheo activations
    rh_a_opt = zeros(N+1,1);
    for i = 1:1
        rh_a_opt(:,i) =w_opt(NParameters+NMuscle+NMuscle+2*nq.all+i:Nwl:Nw);
    end
    ...
    % Rheo activations
    rh_a_opt_ext=zeros(N*(d+1)+1,1);
    rh_a_opt_ext(1:(d+1):end,:)= rh_a_opt;
    for nmusi=1:1
        rh_a_opt_ext(2:(d+1):end,nmusi) = w_opt(Nwm+d*NMuscle+...
            d*NMuscle+d*2*nq.all+nmusi:Nwl:Nw);
        rh_a_opt_ext(3:(d+1):end,nmusi) = w_opt(Nwm+d*NMuscle+...
            d*NMuscle+d*2*nq.all+1+nmusi:Nwl:Nw);
        rh_a_opt_ext(4:(d+1):end,nmusi) = w_opt(Nwm+d*NMuscle+...
            d*NMuscle+d*2*nq.all+1+1+nmusi:Nwl:Nw);
    end
    %% Unscale results
    ...
    % Rheo activations
    rh_a_opt_unsc = rh_a_opt(1:end-1,:).*repmat(...
        scaling.rh_a,size(rh_a_opt(1:end-1,:),1),1);
    ...
    % Rheo excitations
    e_rh_opt_unsc = e_rh_opt.*repmat(scaling.e_rh,size(e_rh_opt,1),...
        size(e_rh_opt,2));
    ...
    % Right leg
    dist_r = sqrt(f_Jnn3(out_res_opt_all(end,foot2Orall.r)-...
        out_res_opt_all(1,foot2Orall.r)));
    % Left leg
    ...
    for i = 1:N    
       ...
       Tau_pass_opt.rheo.r        = f_PassiveMoments(k_pass.rheo.r,...
           theta.pass.rheo.r,Xk_Qs_Qdots_opt(i,jointi.rheo.r*2-1),...
           Xk_Qs_Qdots_opt(i,jointi.rheo.r*2));
        ...
        Tau_pass_opt.prosfoot.r       = f_PassiveMoments(k_pass.prosfoot.r,...
           theta.pass.prosfoot.r,Xk_Qs_Qdots_opt(i,jointi.prosfoot.r*2-1),...
           Xk_Qs_Qdots_opt(i,jointi.prosfoot.r*2));
        ...
        Tau_pass_opt_all(i,:) = full([Tau_pass_opt.hip.flex.l,...
            ...
            Tau_pass_opt.rheo.r,Tau_pass_opt.ankle.l,...
            Tau_pass_opt.subt.l,Tau_pass_opt.prosfoot.r,...
            Tau_pass_opt.mtp.l,Tau_pass_opt.trunk.ext,...
...
if decomposeCost
    ...
    if saveResults              
        ...
        Results_pred.([namescript,'_c',num2str(ww)]).stride_width_std = ...
            stride_width_std;
        Results_pred.([namescript,'_c',num2str(ww)]).foot2_Orig_r = ...
            foot2_Orig_r;
        ...
        Results_pred.([namescript,'_c',num2str(ww)]).MTVel_opt = ...
            vMT_opt_all_new;
        Results_pred.([namescript,'_c',num2str(ww)]).ActsRheo_opt = ...
            rh_a_opt_unsc;
        Results_pred.([namescript,'_c',num2str(ww)]).ExtsRheo_opt = ...
            e_rh_opt_unsc;
