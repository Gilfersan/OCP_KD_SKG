...
% Date: 1/7/2019
%
% Edited by Gilmar Fernandes dos Santos on 14.10.2023
clear all;
clc
close all;
...
idx_ww = [1075]; % Index row in matrix settings (1:198) idx_ww = [x;y];
...
if ispc    
    switch setup.derivatives
        case {'AD'}   
            if cm == 1
                F = external('F','PredSim_HS1.dll');   
                if analyseResults
                    F1 = external('F','PredSim_HS1_pp.dll');
                end
            elseif cm == 2
                F = external('F','PredSim_HS1_SSCM.dll');   
                if analyseResults
                    F1 = external('F','PredSim_HS1_SSCM_pp.dll');
                end
            else
                name_Fext = ['F = external(''F'',''PredSim_HS1_SSCM',num2str(cm),'.dll'');'];
                eval(name_Fext);
                if analyseResults
                    name_Fext1 = ['F1 = external(''F'',''PredSim_HS1_SSCM',num2str(cm),'_pp.dll'');'];
                    eval(name_Fext1);
                end
            end
    end
...
[bounds,scaling] = getBounds_all_Gil4_mtp(Qs_walk,NMuscle,nq,jointi,v_tgt);
